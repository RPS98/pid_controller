...
Vector3d integral_accum_error = 5 * Vector3d::Identity();
Vector3d antiwindup_cte_      = Vector3d::Identity();
double antiwindup_cte_d       = 1.0;
Vector3d _proportional_error  = -1.0 * Vector3d::Identity();

Vector3d resetIntegral1(Vector3d &integral_accum_error_,
...
Vector3d resetIntegral2(Vector3d &integral_accum_error_,
                        const bool &proportional_limitation) {
  if (reset_integral_flag_) {
    for (short j = 0; j < 3; j++) {
      if (std::abs(integral_accum_error_[j]) > antiwindup_cte_[j]) {
