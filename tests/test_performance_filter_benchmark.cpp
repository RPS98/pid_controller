...
Vector3d filtered = Vector3d::Identity();
double alpha_1    = 1.0;
Matrix3d alpha_2  = Matrix3d::Identity();
Vector3d alpha_3  = Vector3d::Identity();
...
Vector3d computeFilter3(Vector3d &new_value) {
  return filtered =
      alpha_3.cwiseProduct(new_value) + (Vector3d::Identity() - alpha_3).cwiseProduct(filtered);
}

static void BM_TEST_FILTER1(benchmark::State &state) {
  Vector3d value = Vector3d::Identity();
  filtered       = Vector3d::Identity();
  for (auto _ : state) {
